...
## D58 — Une police d'une autre origine : l'en-tête CORS est posé sur la réponse, la sécurité n'est pas désactivée

**Choix.** Toute réponse dont Chromium dit qu'elle est une police (`resourceType: Font`) est mise en pause **au retour** par `Fetch` et reçoit `Access-Control-Allow-Origin: *` avant d'atteindre le moteur. Les en-têtes `Content-Encoding`, `Content-Length` et un `Access-Control-Allow-Origin` déjà présent sont retirés au passage : `Fetch.getResponseBody` rend un corps déjà décodé, et le re-servir sous `gzip` donnerait au moteur des octets qu'il ne sait pas lire. Cela vaut pour tout document, qu'il vienne du disque ou d'un serveur. L'interception cesse donc d'être conditionnelle : quand aucune règle ne réclame de pause à l'aller, seules les réponses de police sont mises en pause — `intercept::allow_font`, `Rules::needed` ne décidant plus que de la pause à l'aller.

**Pourquoi.** Chromium récupère une police en mode CORS ; wkhtmltopdf, sous Qt, ne le faisait pas. Un document qui pointe ses `@font-face` sur son propre hôte d'actifs — la forme qu'a toute application qui sert ses polices — perdait donc ses polices chez nous, et la requête refusée comptait pour un échec réseau, ce qui ajoutait une sortie 1. Mesuré sur wkhtmltopdf 0.12.6.1 dans `debian:bookworm-slim`, contre un serveur en boucle locale qui n'envoie aucun en-tête CORS :

| Document | Police | wkhtmltopdf | rchtmltopdf avant |
| --- | --- | --- | --- |
| fichier local | `woff` | dessine avec la police servie, sortie 0 | repli, sortie 1 |
| fichier local | `truetype` | dessine avec la police servie, sortie 0 | repli, sortie 1 |
| fichier local | `woff2` | repli — le format dépasse son Qt —, sortie 0 | repli, sortie 1 |
| servi sur `:8000` | `woff` depuis `:8001` | dessine avec la police servie, sortie 0 | repli, sortie 1 |

La dernière ligne décide de la portée : wkhtmltopdf est indifférent à l'origine du document, donc la règle l'est aussi.

**D'où cela vient.** D'un projet de référence (#32), pas du corpus : une application Symfony qui passe par Snappy et sert ses polices depuis son propre hôte. `pdffonts` sur le PDF de référence nomme les cinq fontes du document ; sur le nôtre, une seule, celle du repli système. Aucun des 72 cas du harnais ne pouvait le voir : leurs fixtures embarquent leur police en `data:`, ce qui est précisément ce qui met une police à l'abri de cette question.
